...
# Updated after alignment
                'raspberry_gonio2':dict(path_IRF = calibration_dir / 'IRF_Flame2OrangeFiber_wLens-F260SMA-A+100um-slit_PMA12-reference.txt',\
                                       abs_calfactor = 6.68E6),\
...
def process_gonio_time_series(folder, current = None, angle_offset = 0.0, calibration = 'raspberry_gonio2', correct_time_drift = True, known_t0 = True):
    """
    ...
    known_t0 : bool, optional
        If True, the t0 is taken from the iv file, else it is taken from the first luminance file.
    ...
    if known_t0:
        iv_files = sorted(list(folder.glob('*.txt')))
        if (len(iv_files) == 0):
            raise Exception('No iv file found.')
        fileiv = iv_files[0]
        t0 = np.loadtxt(fileiv, max_rows = 1, dtype = np.datetime64)
    else:
        t0 = np.loadtxt(filesL0[0], max_rows = 1, dtype = np.datetime64)
...
def plot_voltage_luminance(folder):
    folder = Path(folder)
    # Assuming there is only one txt file and that it is the log
    iv_files = sorted(list(folder.glob('*.txt')))
    if len(iv_files) == 0:
    ...
    files = sorted(list((folder / raw_folder).glob('*_time-series.dat')))
    ...
    lineV, = ax.plot(timeV/tunit, voltageV, 'o-C0', label = 'voltage', markevery = max(len(timeV) //25, 1))
    
    vtimes, vluminances = np.loadtxt(file_luminance, usecols = (0,1), unpack = True)
    
    ax2 = ax.twinx()
    # ax2.set_ylim(0,1750)
    lineL, = ax2.plot(vtimes/tunit, vluminances, 's-C1', label = 'luminance', mfc = 'white', markevery  = max(len(vluminances) // 25, 1))
...
def fit_data_time_series(folder, thickness, error_landscape_file, folder_cal = calibration_dir, ethickness = None):
    ...
    simEL = load_simdata(folder_cal / error_landscape_file, wl_limit=(450,750))
    ...
    temp_list = list((folder / raw_folder).glob('*_time-series.dat'))
    temp_list.sort()
    vtimes = create_times_vector(t0, temp_list)
...
# #------------------ Voltage luminance position plot --------------------------
    ...
    lineL, = ax2.plot(timeL/tunit, luminanceL, 's-C1', label = 'luminance', mfc = 'white', markevery  = max(len(luminanceL) // 25, 1))
