...
def process_goniodata(file, correct_offset = None, current = None, plot = False, path_IRF = path_IRF, path_eye_response = path_eye_response, correct_time_drift = False):
    """
    ...
    correct_offset: None type, str or float/int
        Whether to substract and offset to the angle data or not. If None, nothing is done. If a 'auto' is passed, the offset is automatically derived form the symmetry axis between the left and right hemisphere of the angle resolved data, if a float or integer is passed, that value is consider to be the offset to be substracted. The default is None.
    ...
    correct_time_drift: boolean
        If set to True, the radiant intensity will be corrected for the temporal drift between the first measurement and the last one, based on a linear interpolation using the 3 values taken at 0°.
    ...
    elif  isinstance(correct_offset, int) or isinstance(correct_offset, float):       
        angle_offset = correct_offset
        print(f'INFO: Manual angle offset correction applied. Offset = {angle_offset:.2f}°')
    else:
        print('INFO: No angle offset correction applied.')
        angle_offset = 0.0
