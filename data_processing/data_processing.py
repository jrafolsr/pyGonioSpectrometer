...
def process_goniodata(file, angle_offset = 0.0, current = None, plot = False,\
                      ...
                      path_IRF = path_IRF, path_eye_response = path_eye_response, verbose = True,\
                      folder = None, abs_calfactor = ABS_CALFACTOR, wl_min = 449, wl_max = 801,\
                      saturation_counts = 2**16 -1):
    """
    ...
    abs_calfactor :  float, optional
        Absolute calibration factor from irradiance to luminance. The default is read from the calibration file. 
    wl_min : float, optional
        Min. wavelength cutoff. The default is 449. 
    wl_max : float, optional
            Max. wavelength cutoff. The default is 801.
    saturation_counts : int, optional
        Max. number of counts to be allowed before discarting because it is considered saturated. The default is 2**16-1 = 65535.
    Returns
    -------
    (could be improved, a bit messy)
        iTime, IntTime, Nscans, L0, CE, EQE, Lamb_Corr_Factor, Angles, Wavelengths, SpecRadInt,SpecLumInt,LumInt, LumIntNorm, RadiantIntensity,RadiantIntensityNorm
    ...
    filt_by_saturated = (MeasSpectra <= saturation_counts).all()
    
    if not filt_by_saturated:
        print (f'Warning! FiLe {file.name} contains saturated spectra.')
    ...
    elif  isinstance(angle_offset, (int, float)):
        if verbose:
            print(f'INFO: Manual angle offset correction applied. Offset = {angle_offset:.2f} deg')
    else:
        raise Exception("The argument angle_offset must be either integer, float or 'auto'.")
...
#     print(SpecLumInt.shape, Wavelengths.shape)
    # Find the zeros, assuming the measurement is aligned!
    ...
    LumIntNorm = LumInt / LumInt[zero_a].mean()  # Normalization for the 0 deg  
    
    # Update 10/12/2021, I am going to assume a point-like source, so no need to correct by the area?     
    # Luminance, Projected area correction that gives Luminance [cd m-2]
    Luminance = LumInt / PIXEL_SIZE
    
    # # Norm. luminance,Projected area correction
    # LuminanceNorm = LumIntNorm / np.cos(Angles * np.pi / 180.0)
    
    RadiantIntensity =  np.trapz(SpecRadInt, Wavelengths, axis =  1)
    RadiantIntensityNorm = RadiantIntensity / RadiantIntensity[zero_a].mean() 
    ...
    text += f'# angle_offset = {angle_offset:.2f} deg\n'
    ...
    with open(fintegrated, 'w') as f:
        f.write(text)
    tdata = np.vstack((vTimes, Angles, LumInt, LumIntNorm, RadiantIntensity, RadiantIntensityNorm)).T
                      
    with open(fintegrated, 'a') as f:
        header = 'eTime\t Angles \t LuminousIntensity \t NormLuminousIntensity \t RadiantIntensity\t RadiantIntensityNorm\n'
        header += 's \t deg \t cd \t a.u. \t  W sr-1\t a.u.'
    ...
    with open(fradiometric, 'w') as f:
        ...
        np.savetxt(f, tdata.reshape((1, n+1)), fmt = '%10.2f', header = 'angles (deg)',delimiter='\t')
    ...
    return iTime, IntTime, Nscans, L0,CE, EQE, Lamb_Corr_Factor,\
            ...
            LumInt, LumIntNorm, RadiantIntensity, RadiantIntensityNorm
            


def process_L0(files, t0 = None, path_IRF = path_IRF, path_eye_response = path_eye_response,\
               saturation_counts = 65535, folder = None, abs_calfactor = ABS_CALFACTOR, wl_min = 449, wl_max = 801):
    """
...
def plot_angular_emission(file, thickness = np.nan, ext = 'png', photometric = True):
    """
    ...
    photometric : bool
        If True it plot the luminous intensity (photometric), if False, it plots the radiometric. The default is True.
    ...
    if photometric: 
        angles, x_intensity = np.loadtxt(file_lumint, usecols= (1,3), unpack = True, skiprows=12)
        ax.set_xlabel('Luminous intensity (norm.)', labelpad = -20)
    else:
        angles, x_intensity = np.loadtxt(file_lumint, usecols= (1,5), unpack = True, skiprows=12) 
        ax.set_xlabel('Radiant intensity (norm.)', labelpad = -20)
    ...
    ax.plot(angles*np.pi/180 , x_intensity, 'o', label = f'{thickness:.0f} nm\n'+ '$K_L = $' + f'{LCF:.2f}')
    ax.plot(theta , np.cos(theta), '--', color = 'black', label = 'Lambertian')
    
    ax.legend(fontsize = 'small', bbox_to_anchor=(1, 1.1), ncol =2)
    if photometric:
        fig.savefig(file.parent / (file.stem + '_angular-profile_LuminousIntensity' + '.' + ext), bbox_inches = 'tight')
    else:
        fig.savefig(file.parent / (file.stem + '_angular-profile_RadiantIntensity' + '.' + ext), bbox_inches = 'tight')
